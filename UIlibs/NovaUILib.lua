...
function OrionLib:MakeWindow(WindowConfig)
	...
	-- ── Sidebar ────────────────────────────────────────────────────────────────
	-- Active tab indicator bar — pai definido depois que WindowStuff existir
	local TabIndicator = Create("Frame", {
		Size             = UDim2.new(0, 3, 0, 26),
		Position         = UDim2.new(0, 0, 0, 0),  -- será ajustado dinamicamente
		BackgroundColor3 = T.Accent,
		BorderSizePixel  = 0,
		ZIndex           = 20,  -- alto para ficar acima do ScrollFrame
	}, { Create("UICorner", {CornerRadius = UDim.new(0, 3)}) })
	...
	-- Adicionar o TabIndicator ao MainWindow após ele existir
	-- (pai = MainWindow, posição calculada relativa ao MainWindow)
	TabIndicator.Parent = MainWindow
	...
	local function LoadSequence()
		...
		-- ScreenGui separado para o intro, para não conflitar com ZIndex da window
		...
		Tween(Backdrop,  0.45, {BackgroundTransparency = 1})
		wait(0.5)
		-- destruir o IntroGui por completo, sem deixar rastro
		IntroGui:Destroy()
		MainWindow.Visible = true
	end
	...
	function Functions:MakeTab(TabConfig)
		...
		local function ActivateTab()
			-- task.defer aguarda o próximo frame para AbsolutePosition estar atualizado
			task.defer(function()
				-- posição Y do TabFrame relativa ao MainWindow
				local tabY = TabFrame.AbsolutePosition.Y - MainWindow.AbsolutePosition.Y
				local tabH = TabFrame.AbsoluteSize.Y
				local indH = 22
				local centerY = tabY + (tabH - indH) / 2
				Tween(TabIndicator, 0.2, {
					Position = UDim2.new(0, 0, 0, centerY),
					Size     = UDim2.new(0, 3, 0, indH),
				})
			end)
		end
		...
		local function GetElements(ItemParent)
			...
			function ElementFunction:AddButton(ButtonConfig)
				...
				AddConnection(Click.MouseEnter,      function() Tween(ButtonFrame, 0.18, {BackgroundTransparency = 0.60}) end)
				AddConnection(Click.MouseLeave,      function() Tween(ButtonFrame, 0.18, {BackgroundTransparency = 0.72}) end)
				AddConnection(Click.MouseButton1Down,function() Tween(ButtonFrame, 0.1,  {BackgroundTransparency = 0.50}) end)
				AddConnection(Click.MouseButton1Up,  function()
					Tween(ButtonFrame, 0.18, {BackgroundTransparency = 0.60})
			...
			function ElementFunction:AddToggle(ToggleConfig)
				...
				AddConnection(Click.MouseEnter,      function() Tween(ToggleFrame, 0.18, {BackgroundTransparency = 0.60}) end)
				AddConnection(Click.MouseLeave,      function() Tween(ToggleFrame, 0.18, {BackgroundTransparency = 0.72}) end)
				AddConnection(Click.MouseButton1Up,  function()
					Tween(ToggleFrame, 0.18, {BackgroundTransparency = 0.60})
					SaveCfg(game.GameId)
					Toggle:Set(not Toggle.Value)
				end)
				AddConnection(Click.MouseButton1Down,function() Tween(ToggleFrame, 0.1, {BackgroundTransparency = 0.50}) end)
			...
			function ElementFunction:AddBind(BindConfig)
				...
				AddConnection(Click.MouseEnter,       function() Tween(BindFrame, 0.18, {BackgroundTransparency = 0.60}) end)
				AddConnection(Click.MouseLeave,       function() Tween(BindFrame, 0.18, {BackgroundTransparency = 0.72}) end)
				AddConnection(Click.MouseButton1Up,   function() Tween(BindFrame, 0.18, {BackgroundTransparency = 0.60}) end)
				AddConnection(Click.MouseButton1Down, function() Tween(BindFrame, 0.1,  {BackgroundTransparency = 0.50}) end)
			...
			function ElementFunction:AddTextbox(TextboxConfig)
				...
				AddConnection(Click.MouseEnter,       function() Tween(TextboxFrame, 0.18, {BackgroundTransparency = 0.60}) end)
				AddConnection(Click.MouseLeave,       function() Tween(TextboxFrame, 0.18, {BackgroundTransparency = 0.72}) end)
				AddConnection(Click.MouseButton1Up,   function() Tween(TextboxFrame, 0.18, {BackgroundTransparency = 0.60}); TextboxActual:CaptureFocus() end)
				AddConnection(Click.MouseButton1Down, function() Tween(TextboxFrame, 0.1,  {BackgroundTransparency = 0.50}) end)
			end
